...
export class RequestEditComponent {
    ...
    getRequestDetail(){
        var post = {id : this.requestId };
        this._requestService.getRequestDetailForEdit(post).subscribe(res => {
            var data = res.data.request;
            //this.startDate = data.startDate;
            //this.endDate = data.endDate;

            //this.startDateOptions.initialDate = new Date(data.startDate);
            //this.endDateOptions.initialDate = new Date(data.endDate);
            //let momentObj = moment('05-11-1986', "MM-DD-YYYY");
           // this.startDate.momentObj = momentObj;

            let dateModel:DateModel = new DateModel();
            let momentObj = moment(data.startDate, "YYYY-MM-DD");
            dateModel.momentObj = momentObj;
            dateModel.formatted = momentObj.format("YYYY-MM-DD");
            this.startDate = dateModel;

            let enddt:DateModel = new DateModel();
            let endMomObj = moment(data.endDate, "YYYY-MM-DD");
            enddt.momentObj = endMomObj;
            enddt.formatted = endMomObj.format("YYYY-MM-DD");
            this.endDate = enddt;
            ...
            if(data.otherCategoryTitle){
                ...
                for(var i = 0; i < this.categories.length; i++){
                    if(this.categories[i]._id == data.categoryId){
                        this.selectCategory(this.categories[i]);
                    }
            }
            }
            this.selectedCacpacity = {};
            for(var i = 0; i < this.capacities.length; i++){
                    if(this.capacities[i]._id == data.capacityId){
                        this.selectCapacity(this.capacities[i]);
                    }
            }
            
            //this.selectedCacpacity._id = data.capacityId;



            this.budget = data.budgetId;
            this.description = data.description;
            if(data.timeDuration == "4 Hours" || data.timeDuration == "8 Hours" || data.timeDuration == "12 Hours"){
                this.spaceTime = data.timeDuration;
            }else {
                this.otherSpaceTime = true;
                this.otherSpaceTimeTitle = data.timeDuration;

            }
        }, err => {
             this.messageAlert('error','Sorry You have No Access');
